...
impl Measurements {
    fn parse(data: [u8; BME280_P_T_H_DATA_LEN], calibration: &mut CalibrationData) -> Result<Self, Error> {
    ...
    fn compensate_temperature(uncompensated: u32, calibration: &mut CalibrationData) -> Result<f32, Error> {
    ...
    fn compensate_pressure(uncompensated: u32, calibration: &mut CalibrationData) -> Result<f32, Error> {
    ...
    fn compensate_humidity(uncompensated: u32, calibration: &mut CalibrationData) -> Result<f32, Error> {
...
impl BME280 {
    pub fn new() -> Self {
        BME280 { 
            ...
            delay: Duration::from_millis(SLEEP_TIME),
            calibration: None,
        }
    }
    ...
    pub fn init(&mut self) -> Result<usize, Error> {
        self.dev.set_slave_address(BME280_I2C_ADDR_PRIMARY.into())?;
    ...
    fn soft_reset(&mut self) -> Result<(), Error> {        
        self.write_register(BME280_RESET_ADDR as u8, BME280_SOFT_RESET_CMD)?;
    ...
    fn configure(&mut self) -> Result<usize, Error> {
        match self.mode()? {
            SensorMode::Sleep => {}
            _ => self.soft_reset()?,
        };
    ...
    fn mode(&mut self) -> Result<SensorMode, Error> {
        let mut data: [u8; 1] = [0];
        self.dev.write_read(&[BME280_PWR_CTRL_ADDR], &mut data)?;
    ...
    fn forced(&mut self) -> Result<usize, Error> {
        self.set_mode(BME280_FORCED_MODE)
    }

    fn set_mode(&mut self, mode: u8) -> Result<usize, Error> {
        match self.mode()? {
            SensorMode::Sleep => {}
            _ => self.soft_reset()?,
        };
    ...
    pub fn measure(&mut self) -> Result<Measurements, Error> {
        self.forced()?;
        thread::sleep(self.delay); // await measurement
    ...
    fn read_register(&mut self, register: u8) -> Result<u8, Error> {
        let mut data: [u8; 1] = [0];
        self.dev.write_read(&[register], &mut data)?;
        Ok(data[0])
    }
    ...
    fn read_pt_calib_data(&mut self, register: u8) -> Result<[u8; BME280_P_T_CALIB_DATA_LEN], Error> {
    ...
    fn write_register(&mut self, register: u8, payload: u8) -> Result<usize, Error> {
        let written = self.dev.write(&[register, payload])?;
        Ok(written)
    }
}
